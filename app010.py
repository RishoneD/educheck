...
components.html(
    ...
    "if(!window.parent.document.getElementById('gradify-feedback-float')){"
    "  var fb=window.parent.document.createElement('button');"
    "  fb.id='gradify-feedback-float';"
    "  fb.title='שלח משוב';"
    "  fb.textContent='💬';"
    "  fb.style.cssText='position:fixed;bottom:24px;right:24px;z-index:9990;"
    "width:50px;height:50px;border-radius:50%;background:#15876a;color:#fff;"
    "border:none;font-size:22px;cursor:pointer;"
    "box-shadow:0 3px 14px rgba(21,135,106,.4);line-height:1;';"
    "  fb.addEventListener('click',function(){"
    "    var btns=window.parent.document.querySelectorAll('button');"
    "    for(var i=0;i<btns.length;i++){"
    "      if(btns[i].textContent.trim()==='💬'&&btns[i].id!=='gradify-feedback-float'){"
    "        btns[i].click();break;"
    "      }"
    "    }"
    "  });"
    "  window.parent.document.body.appendChild(fb);"
    "}"
    # מסתיר את כפתור הפרוקסי (אך נשאר קליקבל תוכניתית)
    "function hideProxyFeedbackBtn(){"
    "  var btns=window.parent.document.querySelectorAll('button');"
    "  for(var i=0;i<btns.length;i++){"
    "    if(btns[i].textContent.trim()==='💬'&&btns[i].id!=='gradify-feedback-float'){"
    ...
    "        el=el.parentElement;"
    "      }"
    "      if(el)el.style.cssText='position:fixed!important;top:-9999px!important;left:-9999px!important;width:1px!important;height:1px!important;overflow:hidden!important;';"
    "      break;"
    "    }"
    "  }"
    "}"
    "var proxyObs=new MutationObserver(hideProxyFeedbackBtn);"
    "proxyObs.observe(window.parent.document.body,{childList:true,subtree:true});"
    "hideProxyFeedbackBtn();"
    "</script>",
    height=0,
    scrolling=False,
)
...
_UPDATE_VERSION = os.environ.get('GRADIFY_UPDATE', '')
_DOWNLOAD_URL   = os.environ.get('GRADIFY_DOWNLOAD_URL', '')
if _UPDATE_VERSION:
    _update_banner_html = (
        '<div id="gradify-update-banner" style="'
        'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);'
        'background:#fff;border:1.5px solid #d4e4f1;border-radius:14px;'
        'padding:14px 18px;box-shadow:0 4px 24px rgba(0,0,0,.13);'
        ...
        '<div style="flex:1;">'
        f'<div style="font-weight:700;font-size:14px;color:#243029;">גרסה {_UPDATE_VERSION} זמינה!</div>'
        f'<a href="{_DOWNLOAD_URL}" target="_blank" style="font-size:12.5px;color:#2563a8;'
        'text-decoration:underline;text-underline-offset:2px;">לחצו כאן להורדה</a>'
        '</div>'
        '<button onclick="document.getElementById(\'gradify-update-banner\').style.display=\'none\'" '
        'style="background:none;border:none;cursor:pointer;font-size:19px;'
        'color:#9aa69e;padding:2px 5px;line-height:1;flex-shrink:0;">✕</button>'
        '</div>'
    )
    components.html(
        "<script>"
        "if(!window.parent.document.getElementById('gradify-update-banner')){"
        "  var tmp=window.parent.document.createElement('div');"
        "  tmp.innerHTML=" + repr(_update_banner_html) + ";"
        "  window.parent.document.body.appendChild(tmp.firstElementChild);"
        "}"
        "</script>",
        height=0,
...
# ── Feedback ──────────────────────────────────────────────────────────────────
@st.dialog('💬 שלחו לנו משוב', width='large', dismissible=False)
def _feedback_dialog():
    st.markdown(
        '<p style="color:#6f7d74;font-size:14px;margin:0 0 18px;">'
        'כל הערה, הצעה או תקלה — שמחים לשמוע.</p>',
        unsafe_allow_html=True,
    )

    st.radio(
        'סוג הפנייה',
        options=['improvement', 'bug'],
        format_func=lambda x: '💡 הצעת שיפור' if x == 'improvement' else '🔧 דיווח תקלה',
        horizontal=True,
        key='fb_category',
    )
    st.text_input('שם המורה (לא חובה)', key='fb_teacher', placeholder='לדוגמה: גב׳ כהן')

    # מציע פרטי כיתה אם נמצאים בדף תוצאות
    _ctx_default = ''
    if st.session_state.get('class_name'):
        _type_label = 'תקופתי מחצית' if st.session_state.get('report_type') == 'semester' else 'תקופתי שנתי'
        _ctx_default = f'כיתה {st.session_state.class_name}, {_type_label}'
    st.text_input('כיתה וסוג דוח (לא חובה)', key='fb_context',
                  value=_ctx_default,
                  placeholder='לדוגמה: כיתה י׳1, תקופתי מחצית')

    st.text_area('תיאור *', key='fb_text', height=130,
                 placeholder='תארו מה קרה / מה הייתם רוצים לשפר...')

    st.markdown(
        '<style>'
        '[data-testid="stDialog"] [data-testid="stBaseButton-secondary"]{'
        'background:#fff!important;color:#6f7d74!important;border:1px solid #e3e8e2!important;'
        'font-size:14px!important;padding:10px!important;box-shadow:none!important;}'
        '[data-testid="stDialog"] [data-testid="stBaseButton-secondary"]:hover{'
        'background:#f4f6f3!important;color:#243029!important;}'
        '</style>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    with col_send:
        if st.button('שלח →', key='fb_send', type='primary', use_container_width=True):
            text = st.session_state.get('fb_text', '').strip()
            if not text:
                st.error('נא לרשום תיאור לפני שליחה.')
            else:
                from core.feedback_sender import send_feedback
                ctx  = st.session_state.get('fb_context', '').strip()
                body = f'[{ctx}]\n{text}' if ctx else text
                ok, err = send_feedback(
                    st.session_state.get('fb_category', 'improvement'),
                    st.session_state.get('fb_teacher', ''),
                    body,
                )
                if ok:
                    ...
                    st.error(err)
    with col_cancel:
        if st.button('ביטול', key='fb_cancel', use_container_width=True, type='secondary'):
            st.session_state.feedback_open = False
            st.rerun()
...
# כפתור פרוקסי בלתי-נראה — מופעל ע"י כפתור ה-💬 הצף שב-DOM
st.markdown('<div class="btn-feedback-proxy">', unsafe_allow_html=True)
_proxy_clicked = st.button('💬', key='feedback_trigger_btn')
st.markdown('</div>', unsafe_allow_html=True)

if _proxy_clicked and not st.session_state.get('feedback_open'):
    st.session_state.feedback_open = True

# הדיאלוג צף מעל התוכן הרגיל
